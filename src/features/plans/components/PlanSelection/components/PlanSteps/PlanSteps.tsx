...
export function PlanSteps({ steps, activeStepIndex }: PlanStepsProps) {
    ...
    const currentIndex = hasSteps ? Math.min(Math.max(activeStepIndex, 0), totalSteps - 1) : 0;
        ...
        <>
            <ol className={styles.steps}>
                {steps.map((step, index) => {
                    ...
                    return (
                        <li key={step.number} className={styles.step} data-active={isActive}>
                            <span className={styles.stepNumber} aria-hidden="true">
                                {step.number}
                            </span>
                            <span className={styles.stepLabel}>{step.label}</span>
                            {index < steps.length - 1 && <span className={styles.stepDivider} aria-hidden="true" />}
                        </li>
                    );
                })}
            </ol>
            ...
            <div className={styles.stepsMobile}>
                <span className={styles.stepsMobileLabel}>PASO {currentStepNumber} DE {totalSteps || 1}</span>
                <div
                    className={styles.stepsMobileBar}
                    role="progressbar"
                    aria-valuemin={1}
                ...
                >
                    <span className={styles.stepsMobileBarFill} style={{ width: `${progressPercentage}%` }} />
                </div>
            </div>
        </>
    );
